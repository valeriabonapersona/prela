...
facet_order_acute <- c("behavior", "plasma", "amyg IEG", "hip BDNF")
out_order_acute <- c("anxiety", "non stress. \n learning", "stress. \n learning", 
               "plasma epi \n at rest",
               "cfos \n at rest", "cfos \n after stress", "BDNF \n at rest", 
               "BDNF \n after stress")
g_anxiety_prone <- anxietyprone_df %>% 
  mutate(
    my_facet = case_when(
      ...
      str_detect(out_grouped, "cfos") ~ "amyg IEG", 
      str_detect(out_grouped, "plasma") ~ "plasma",
      str_detect(out_grouped, "bdnf") ~ "hip BDNF",
    ...
    out_grouped = case_when(
      ...
      out_grouped == "nonstressful_learning" ~ "non stress. \n learning",
      out_grouped == "stressful_learning" ~ "stress. \n learning",
  ...
  ggplot(aes(factor(out_grouped, levels = out_order_acute), estimate, fill = at_death_grouped)) + 
  ...
  my_theme + 
  facet_grid(~factor(my_facet, levels = facet_order_acute), scales = "free_x", space = "free_x") + 
